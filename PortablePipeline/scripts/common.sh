...
IMS=$((for line in "${PP_SCRIPTS[@]}"; do
 ...
 #IM_CONFIG_はUbuntuでim-configと重なって挙動が変になる
 set |grep ^IM_|grep -v ^IM_CONFIG_|cut -f 2 -d =)|sort|uniq)
echo "Containers: "$IMS
...
for i in `set |grep ^IM_|grep -v ^IM_CONFIG_|cut -f 1 -d =|sed 's/^IM_//'`; do export ENV_$i="$CON"$(eval "echo \$IM_`echo $i`")" "; done
shopt -s expand_aliases
#Docker使用時に$CONの中に;が入っていてパイプが途中で切れてしまうので、その対策
if [ "$CON" = "$CON_DOCKER" ]; then
 for i in `set |grep ^IM_|grep -v ^IM_CONFIG_|cut -f 1 -d =|sed 's/^IM_//'`; do alias DO_$i="FUNC_RUN_DOCKER "$(eval "echo \$IM_`echo $i`")" "; done
 for i in `set |grep ^IM_|grep -v ^IM_CONFIG_|cut -f 1 -d =|sed 's/^IM_//'`; do alias DOGPU_$i="FUNC_RUN_DOCKER_GPU "$(eval "echo \$IM_`echo $i`")" "; done
elif [ "$CON" = "$CON_PODMAN" ]; then
 for i in `set |grep ^IM_|grep -v ^IM_CONFIG_|cut -f 1 -d =|sed 's/^IM_//'`; do alias DO_$i="FUNC_RUN_PODMAN "$(eval "echo \$IM_`echo $i`")" "; done
 for i in `set |grep ^IM_|grep -v ^IM_CONFIG_|cut -f 1 -d =|sed 's/^IM_//'`; do alias DOGPU_$i="FUNC_RUN_PODMAN "$(eval "echo \$IM_`echo $i`")" "; done
else
 #singularity
 for i in `set |grep ^IM_|grep -v ^IM_CONFIG_|cut -f 1 -d =|sed 's/^IM_//'`; do alias DO_$i="$CON"$(eval "echo \$IM_`echo $i`")" "; done
 for i in `set |grep ^IM_|grep -v ^IM_CONFIG_|cut -f 1 -d =|sed 's/^IM_//'`; do alias DOGPU_$i="$CON_GPU"$(eval "echo \$IM_`echo $i`")" "; done
fi
