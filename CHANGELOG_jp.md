...
# 1.6.0 (2026/04/21)
- GUIでスクリプトのカテゴリ表示、設定画面の名前付き保存
- pp.py直接実行時に、解析スクリプトからのエラーコードを伝え、異常終了の場合はエラーコード1で終了するようにした。
- シンボリックリンク先のpathを探す際に、現在のフォルダー以下の2階層までのすべてのファイルを対象にするのではなく、```input_*, opt_*```に指定されたファイル、フォルダーだけを対象に探すようにした。
- ```Comparative-genomics~make-align-for-mauve```でwhile readの中でDO_ aliasを使用していてループが１回で切れていたのでfor ループに変更した。

# 1.5.6 (2026/04/03)
- ```nanopore~get-consensus```でアウトプットのExcelファイルを作成し、コンセンサス配列に対して元リードのマルチプルアライメントの様子をグラフィカルに可視化するHTMLを出力。
- ```nanopore~split-barcode```でリード長分布を対数スケールの塩基長vs塩基数でプロットする機能を追加。
- Macのdmgパッケージ版で、ssh接続ができないバグを修正。
- CUIでのインタラクティブな操作モードを追加。
 
# 1.5.5 (2026/02/26)
- ```nanopore~get-consensus```で一覧表のリードカウントを集計する際に、blastのトップヒットではなく、同一スコア・同一カバー率のヒット数を考慮した重みで割って集計するように変更。
- Ubuntu 22.04などでOSで使用している環境変数IM_CONFIG_XXXと重なってコンテナイメージを間違ってしまいエラーになっていたのを修正。

# 1.5.4 (2026/02/03)
- ```nanopore~get-consensus```の最小クラスターリード数を10->3に変更, 2, 3回目のクラスタリングの閾値を別途設定するように変更。3回目のクラスタリングはvsearchだとまとめ切れていないことがあったので、cd-hitを使うように変更。blastの一致率の閾値も別途設定するように変更。2回目のクラスタリングの後で、サンプル内のリードをマッピングしてリードバックフェージングを行いハプロタイプを分ける機能を追加。
