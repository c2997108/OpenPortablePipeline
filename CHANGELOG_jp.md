...
- armネイティブなdockerコンテナも併せて登録する方針に変更。強制的に`linux/amd64`イメージをロードするようにしていたけど、その指定を外した。`assemble~megahit`,`QC~jellyfish`,`assemble~hifiasm`,`nanopore~get-consensus`,`ZZZ~hello-world`,`ZZZ~wait-10sec`,`ZZZ~wait-1min`
